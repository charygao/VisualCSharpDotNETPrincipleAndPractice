...
namespace ModifyDataBase
{
	...
	public class config
	{
		...
		{   
			...
			//断开SchoolManage的一切连接
			string str_Database=ConfigurationSettings.AppSettings["Database"].ToString();
			string str_Sql_DisConnect;
			str_Sql_DisConnect="declare hcforeach cursor global for select 'kill '+rtrim(spid) from master.dbo.sysprocesses where dbid=db_id('"
		...
		public  bool School_ID_Had(string str_School_ID)
		{
			string str_Sql="select * from School where School_ID='"+str_School_ID+"'";
			//if(GetRowRecord(str_Sql)==false)return false;
			if(GetRowCount(str_Sql)==0)return false;
			return true;
		}
