...
namespace DataBaseSet
{
	...
	public class config
	{
		...
		//测试数据连接
		public string TestOpen(string DatabaseServer,string DatabaseUser,string DatabasePassword)
		{  
			string myConnectionStr="Provider=SQLOLEDB.1;Persist Security Info=False;Use Procedure for Prepare=1;Auto Translate=True;Packet Size=4096;Workstation ID=DH;Use Encryption for Data=False;Tag with column collation when possible=False;Initial Catalog=Master;"
		...
		{  
			string myConnectionStr="Provider=SQLOLEDB.1;Persist Security Info=False;Use Procedure for Prepare=1;Auto Translate=True;Packet Size=4096;Workstation ID=DH;Use Encryption for Data=False;Tag with column collation when possible=False;Initial Catalog=Master;"
				+"Data Source="+SystemInformation.ComputerName+ConfigurationSettings.AppSettings["DatabaseServer"]
			...
			string str_Sql_DeleteDB="DROP DATABASE CityEduManage";
			try
			{
				ExeSql(str_Sql_DeleteDB);
			}
			catch(Exception e)
			{
				string errorMessage = e.Message;				
		...
		/// <summary>
		/// 绑定comboBox控件并显示数据,ComboBox控件Value,Text值将分别等于等于str_Value,str_Text值
		/// </summary>
		/// <param name="str_Value">绑定ComboBox控件Value值相对应数据库表字段名</param>
		/// <param name="str_Text">绑定ComboBox控件Text值相对应数据库表字段名</param>
		/// <param name="sql">Select-SQL语句</param>
		/// <param name="myDropDownList">ComboBox控件id值</param>
		public string BindComboBox(string str_Sql,ComboBox myComboBox,string theValueMember ,string theDisplayMember)
		{
			string errorstring=Fill(str_Sql);
			if(errorstring!="OK")return errorstring;
		
			myComboBox.DataSource=ds.Tables[0].DefaultView;
			myComboBox.ValueMember=theValueMember;
			myComboBox.DisplayMember =theDisplayMember;
			return "OK";			
		}
		...
		/// <param name="sql">Select-SQL语句</param>
		/// <param name="myDropDownList">ComboBox控件id值</param>
		public string BindlistBox(string str_Sql,ListBox mylistBox,string theValueMember ,string theDisplayMember)
		{
			string errorstring=Fill(str_Sql);
			if(errorstring!="OK")return errorstring;
			
			mylistBox.DataSource=ds.Tables[0].DefaultView;
			mylistBox.ValueMember=theValueMember;
			mylistBox.DisplayMember =theDisplayMember;
			return "OK";			
		}
		...
		public  string Committee_IDtoWhat(string str_Committee_ID,string what)
		{
			string str_Sql="select "+what+" from Committee where Committee_ID="+str_Committee_ID;
			if(GetRowRecord(str_Sql)==false)return "";
			return  dr[what].ToString();
		}
		...
		public  string Teacher_IDtoWhat(string str_Teacher_ID,string what)
		{
			string str_Sql="select "+what+" from Teacher where Teacher_ID='"+str_Teacher_ID+"'";
			if(GetRowRecord(str_Sql)==false)return "";
			return  dr[what].ToString();
		}
		public  bool Teacher_ID_Had(string str_Teacher_ID)
		{
			string str_Sql="select * from Teacher where Teacher_ID='"+str_Teacher_ID+"'";
			//if(GetRowRecord(str_Sql)==false)return false;
			if(GetRowCount(str_Sql)==0)return false;
			return true;
		}
		...
		public  string UserName_ToWhat(string str_UserName,string what)
		{
			string str_Sql="select "+what+" from Users where UserName='"+str_UserName+"'";
			if(GetRowRecord(str_Sql)==false)return "";
			return  dr[what].ToString();
		}
		...
		/// <param name="strSymbol">符号</param>
		/// <returns>返回值</returns>
		public string GetFirstBackStr(string strOriginal,string strSymbol)
		{
			int strPlace=strOriginal.IndexOf(strSymbol);
			if (strPlace!=-1)
				strOriginal=strOriginal.Substring(strPlace+strSymbol.Length);
			return strOriginal;
		}
		...
		//产生有有效密码
		public string ValidUploadPassword(string str_School_ID)
		{
			int strIndex,int_UploadPassword=0;
			string str_UploadPassword="";
			for(strIndex=0;strIndex<str_School_ID.Length;strIndex++)
				int_UploadPassword+=(int)(str_School_ID[strIndex]);
			str_UploadPassword=int_UploadPassword.ToString();
			return str_UploadPassword;			
		}
